...
for i in tqdm(range(epoch)):
    ...
    for cat_train, label_train in dataloader_train:
        ...
        onehot_label = torch.eye(2).to(device)[label.long(), :].squeeze()        #独热编码且删除长度为1的维度
        output = model_tcrep(cat_tensors)
    ...
    with torch.no_grad():
        for cat_val, label_val in dataloader_val:
            ...
            out_put = model_tcrep(cat_tensors)
    ...
    if i == epoch-1:
        torch.save(best_model, 'model_save/GRUemb.pth')
        draw_roc(fpr, tpr, roc_auc, 'val_roc')
