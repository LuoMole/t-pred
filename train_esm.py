...
parser.add_argument('-b', '--batchsize', default=2,  type=int, help="Specify the batchsize")
...
model = AutoModelForSequenceClassification.from_pretrained("facebook/esm1b_t33_650M_UR50S")
esmmodel = model.to(device)
# model_path = r'C:\Users\86875\Desktop\learning\t-pred\code\model_save\kernelsize18.pth'
# model_tcrep = torch.torch.load(model_path)

# momentum = 0.9
optm_tcrep = Adam(esmmodel.parameters(), lr_initial)
...
for i in tqdm(range(epoch)):
    ...
    with torch.no_grad():
        for pair_val, label_val in dataloader_train:
            ...
            # print(token_pair)
            out_put = esmmodel(**token_pair)
            out_put = out_put.logits
            out_put = torch.softmax(out_put, dim=1)
            # out_put = out_put.last_hidden_state[:, 0, :]
    ...
    if i == epoch-1:
        torch.save(best_model, 'model_save/esm.pth')
        draw_roc(fpr, tpr, roc_auc, 'val_roc')
