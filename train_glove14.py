...
parser.add_argument('-b', '--batchsize', default=64,  type=int, help="Specify the batchsize")
...
parser.add_argument('--datastoragefolder', default=r'C:\Users\86875\Desktop\learning\t-pred\code\ming\data-sampled', help="datastoragefolder")
parser.add_argument('--patience4earlystop', default=20, type=int, help="patience for earlystop")
...
aa_tcr = pk.load(open(r'emb\aa_glove.pk', 'rb'))
aa_ep = pk.load(open(r'emb\aa_glove.pk', 'rb'))
# for blosum50
...
data_filenamelist = get_dirlist(data_storeage_folder, 'train','14')
...
train_data, val_data = train_test_split(file_all, test_size=0.2, random_state=42)

# 假设你有一个名为MingDataset的数据集类，以及aa_tcr, aa_ep
...
dataloader_val = DataLoader(dataset_val, batch_size, shuffle=False, drop_last=False)
best_model = None
print('开始训练')
best_f1 = 0
for i in tqdm(range(epoch)):
    ...
    if i == epoch - 1:
        torch.save(best_model, 'model better/glove gru/golve14.pth')
        mcc_val.append(mcc)
        f1_val.append(f1)
        acc_val.append(acc)
