...
parser.add_argument('-b', '--batchsize', default=16,  type=int, help="Specify the batchsize")
...
bertmodel = BertModel.from_pretrained("Rostlab/prot_bert")
bertmodel.to(device)


class Bertcls(nn.Module):
    def __init__(self,cat_size=1024, dropout=0):
        super().__init__()
...
for i in tqdm(range(epoch)):
    ...
    if i == epoch-1:
        torch.save(best_model, 'model_save/protbert.pth')
        draw_roc(fpr, tpr, roc_auc, 'val_roc')
